...
rum.sw1 = FALSE #If TURE run Spearman function on datasets
rum.sw2 = FALSE #If TURE run vector similarity function on datasets
rum.sw3 = FALSE #If TURE run mean squared difference function on datasets
if(run.sw1){
...
```{r}
results<-read.table("/Users/duanshiqi/Desktop/results.txt",header=T)
results
```
Based on the results we obtained, we conlcude that for dataset2, Spearman + No Variance Weighting + Combination of Weight Threshold and Best_n_neighbors is the best model with respect to ROC, and Cluster Model is the best model with respect to MAE. Since we consider people are more interested in whether the algorithm could predict the score of an item correctly with repect to direction(visited or not, high score 4-6 or low score 1-3). If people are more interested in whether the algorithm could predict the scores with respect to how close it is to the actual score, they should consider more on MAE.
