...
```{r}
#######################################
# Helper 2: evaluate(train, test, weight, neighbor, alpha)
...
SCORE_VecSim_VW = evaluate(train1, test1, VecSim_VarW, VecSim_VW_thresh)
...
NS_score
```
Spearman with combo is very close to Spearman with Weight Threshold.
...
scores[order(scores[,2],decreasing=T),]

print("Best score when alpha = 10")
scores[order(scores[,3],decreasing=T),]

```

Conclusion:
"Vector Similarity + no variance weighting (VW) + Weight Threshold neighbor selection (NS)""
is the best.
